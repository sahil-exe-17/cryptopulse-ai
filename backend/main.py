...
def login_user(user: UserAuth):
    try:
        ...
        if not row:
            # Auto-register hack to handle Vercel ephemeral DB resets
            hashed_password = pwd_context.hash(user.password)
            display_name = user.email.split('@')[0]
            c.execute("INSERT INTO users (email, password, display_name, default_currency) VALUES (?, ?, ?, ?)", (user.email, hashed_password, display_name, "USD"))
            conn.commit()
        else:
